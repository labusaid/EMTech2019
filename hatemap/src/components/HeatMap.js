...
function addTweetData(comp, location, keyword, tone){
    Axios.get('http://localhost:3000/getaveragetone?location=' + location + '&keyword=' + keyword)
        .then(response => {
            console.log(response);
            var toneCount = tone ? response.data.positive_count : response.data.negative_count;
            var toneAverage = tone ? response.data.positive_average : response.data.negative_average;
...
class HeatMap extends Component {
    ...
    constructor(props) {
        ...
        addTweetData(this, 'losangeles', 'trump', false);
        addTweetData(this, 'oklahomacity', 'trump', true);
    }
